...
Подходит для тестов и быстрого запуска.
...
Режим включается в конфигурации:
```bash
...
Репозитории автоматически переключаются между RAM и JSON в зависимости от этого параметра.
...
#### GameControllerTest
- [ ] Тест: `GET /api/games/{id}` — возвращает нужную игру и 404 для несуществующего id
- [ ] Тест: `PUT /api/games/{id}` — корректно обновляет игру (проверить новое `title`/`platform`)
- [ ] Тест: `DELETE /api/games/{id}` — возвращает 204/200 и после удаления игра не находится

#### GameControllerValidationTest
- [ ] Тест: пустой `genre` → 400 + сообщение об ошибке
- [ ] Тест: `releaseYear` в будущем → 400

#### SessionControllerTest
- [ ] Тест: `GET /api/sessions/{id}` — вернуть одну сессию по id
- [ ] Тест: `GET /api/sessions/game/{gameId}` — только сессии указанной игры
- [ ] Тест: `DELETE /api/sessions/{id}` — успешное удаление сессии

#### StatsControllerTest
- [ ] Тест: `GET /api/stats/avg-session` — средняя длительность > 0
- [ ] Тест: `GET /api/stats/top-time?limit=3` — возвращает ровно 3 игры, отсортированные по времени
- [ ] Тест: `GET /api/stats/genres-count` — жанры не пустые, количество ≥ 0

#### Service layer
- [ ] Дополнить `StatsServiceTest` edge-кейсами:
- [ ] Пустые списки игр/сессий → корректные значения (0, пустые коллекции)
- [ ] Несколько игр с одинаковым временем → правильный порядок в топе
